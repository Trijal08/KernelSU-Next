...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU-Next" || git clone https://github.com/Levaac/KernelSU-Next && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU-Next"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout dev-susfs && echo "[-] Switched to dev-susfs branch."
    fi
